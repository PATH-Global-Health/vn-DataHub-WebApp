...
const initialState: State = {
  statusMap: {
    ...
    [CANCELED_BY_CONSUMER]: { color: 'grey', label: 'Bên hẹn huỷ' },
    [NOT_DOING]: { color: 'brown', label: 'Không thực hiện' },
    [CANCELED_BY_PROVIDER]: { color: 'red', label: 'Huỷ' },
  },
...
const getTickets = createAsyncThunk(
  'csyt/telemedicine/schedule/getTickets',
  async (arg: { from: Date; to: Date }) => {
    const { from, to } = arg;
    const result = await telemedicineService.getSchedules(from, to);
    return result.map((e) =>
      e.Status === UNFINISHED &&
...
const getTicketFiles = createAsyncThunk(
  'csyt/telemedicine/schedule/getTicketFiles',
...
const slice = createSlice({
  name: 'csyt/telemedicine/schedule',
  initialState,
  reducers: {
